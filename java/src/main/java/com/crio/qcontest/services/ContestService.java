...
public class ContestService{
    ...
    public Contestant createContestant(Long contestId, String userName) {
        ...
        if(contest==null || contest.getContestStatus()==ContestStatus.ENDED)
        {
            throw new RuntimeException("Invalid contest");
        }
    ...
    public List<Contestant> contestHistory(Long contestId) {
        ...
        if(contest == null)
        {
            throw new RuntimeException("No contest found");
        }
        if(contest.getContestStatus() == ContestStatus.IN_PROGRESS || contest.getContestStatus() == ContestStatus.NOT_STARTED)
        {
            throw new RuntimeException("Contest not ended");
        }

        List<Contestant> contestants = contestantRepository.findAllByContestId(contestId);
        System.out.println(contestants.size());
        Collections.sort(contestants,(p1,p2)->{ return Integer.compare(p2.getCurrentContestPoints(), p1.getCurrentContestPoints());});
